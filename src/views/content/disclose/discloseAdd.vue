<template>
  <div class="discloseAdd">
    <el-form ref="editForm" :model="editForm" :rules="editRules" label-width="100px" style="width: 60%;min-width: 800px">
      <el-form-item label="信息分类：" class="infotype" v-if="!this.id" prop="dicId">
        <el-select v-model="editForm.dicId" placeholder="--请选择--" >
      ...
      <el-form-item label="封面图片：" class="imgupload" prop="picListUrls">
      ...
      <el-form-item label="政策内容：" class="editor-item-con" prop="details" ref="contentRef">
        <el-input v-model="editForm.details" v-show="false"></el-input>
        <!-- 图片上传组件辅助-->
      ...
      <el-form-item>
        <el-button type="primary" :disabled="isdisable" size="medium" @click="submitForm('editForm')" style="width:150px">发布</el-button>
      </el-form-item>
    </el-form>
  </div>
</template>
      ...
      this.$refs[formName].validate(async (valid) => {
        if (valid) {
          ...
          const res = await this.$http.policySave({
            ...
            file_url: this.fileImgList[0].url ? this.fileImgList[0].url : "",
          });
          if (res.status === "success") {
            this.$message({
              type: "success",
