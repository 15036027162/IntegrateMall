        ...
        <el-form ref="editForm" :model="editForm" :rules="editRules" label-width="100px" style="width: 60%;min-width: 800px">
            ...
            <el-form-item label="跳转方式：" prop="way" class="waydiv">
                <el-select v-model="editForm.way" placeholder="--请选择--">
                    ...
                    <el-option label="内部模块-志愿者招募" :value="3"></el-option>
                    <el-option label="内部模块-心愿墙" :value="4"></el-option>
            ...
            <div class="urldiv">
                ...
                <el-form-item prop="innerId" v-if="editForm.way == 2" class="innerdiv">
                    <el-select v-model="editForm.innerId" placeholder="--请选择--">
                        <el-option label="--请选择--" value></el-option>
                        <el-option v-for="item in innerList" :key="item.id" :label="item.title" :value="item.id"></el-option>
                    </el-select>
                </el-form-item>
            </div>
    ...
    export default {
        data() {
            return {
                ...
                editRules: {
                    adv_name: [
                        {required: true, min: 0, max: 20, message: "请输入20字以内的主题名称", trigger: "blur",},
                    ],
                    url: [
        ...
        methods: {
            ...
            // 获取列表数据
            async getinnerSelectList(id) {
                id = id+''
                const res = await this.$http.innerSelectList({tabId:id});
                if (res.status === "success") {
            ...
            async getListData() {
                ...
                if (res.status === "success") {
                    ...
                    this.editForm.dicId = Number(resData[0].tabId)
                    this.editForm.innerId = Number(resData[0].innerId)
                    this.getinnerSelectList(this.editForm.dicId)
                    this.fileImgList = [
                        {
