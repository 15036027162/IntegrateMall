        ...
        methods: {
            ...
            async getInfo(){
                ...
                if(res.status==='success'){
                    let data = res.data[0]
                    this.addHonerForm.title = data.title
                    this.addHonerForm.status = data.status
                    this.addHonerForm.details = data.content
                    if (data.picUrl) {
                        this.addHonerForm.picListUrls = data.picUrl
            ...
            async releaseHonerForm(addHonerForm) {
                this.$refs[addHonerForm].validate(async (valid) => {
                    if (valid) {
                        let res = await this.$http.monthlyAwardUpdateAll({
