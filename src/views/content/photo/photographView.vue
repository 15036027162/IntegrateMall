...
<template>
  <div class="integralRegisterView">
    <div class="viewItem">
      <span class="labeltext">影集分类：</span>
    ...
    <div class="viewItem">
      <span class="labeltext">图片描述：</span>
      <div class="content">{{details.name}}</div>
    </div>
     <div class="viewItem">
      <span class="labeltext">点赞数量：</span>
      <div class="content">{{details.likeCount}}</div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    // 获取列表数据
    async getListData() {
      const res = await this.$http.villageAlbumInfo({
        id: this.id || 1,
      });
