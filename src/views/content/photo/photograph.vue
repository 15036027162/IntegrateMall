...
<template>
  <div class>
    <el-form :inline="true" :model="searchForm" class="demo-form-inline">
      ...
      <el-form-item label="影集分类：">
        <el-select v-model="searchForm.type" placeholder clearable>
      ...
      <el-form-item label="内容说明：">
        <el-input v-model="searchForm.name" placeholder="请输入内容说明" clearable></el-input>
      </el-form-item>
      <el-form-item>
        <el-button type="primary" size="medium" @click="goSearch">查询</el-button>
        <el-button type="success" size="medium" @click="handleAdd" v-if="isAuth('web:villagephoto:add')&&deptLevel==5">新增</el-button>
        <el-button size="medium" @click="resetForm">重置</el-button>
      </el-form-item>
    </el-form>
    <el-table border v-loading="dataListLoading" :data="dataList" style="width: 100%">
      <el-table-column prop="typeExplain" label="影集分类"></el-table-column>
      ...
      <el-table-column prop="name" label="内容说明" width="200"></el-table-column>
      <el-table-column prop="likeCount" label="点赞数量"></el-table-column>
      <el-table-column prop="createTime" label="发布时间"></el-table-column>
      <el-table-column label="操作" width="180">
        <template slot-scope="scope">
          <div>
            <el-button type="text" size="small" @click="handleView(scope.row)">查看</el-button>
            <el-button type="text" size="small" @click="handleEdit(scope.row)" style="color:#E6A23C" v-if="isAuth('web:villagephoto:edit')&&deptLevel==5">编辑</el-button>
            <el-button type="text" size="small" @click="handleDelete(scope.row)" style="color:#F56C6C" v-if="isAuth('web:villagephoto:delete')&&deptLevel==5">删除</el-button>
          </div>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    ...
    async getListData() {
      ...
      const res = await this.$http.villageAlbumList({
    ...
    handleDelete(item) {
      this.$confirm("删除后不可恢复，您确定要继续吗?", "删除提示", {
        ...
        .then(async (_) => {
          const res = await this.$http.villageAlbumDelete({
            id: item.id,
          });
          if (res.code == 0) {
            this.$message({
