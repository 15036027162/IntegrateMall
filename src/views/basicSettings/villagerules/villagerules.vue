...
<template>
  <div class>
    <el-form :inline="true" :model="searchForm" class="demo-form-inline" >
      ...
      <el-form-item>
        <el-button type="primary" size="medium" @click="goSearch" v-if="this.deptLevel!=5">查询</el-button>
        <el-button type="success" size="medium" @click="handleAdd" v-if="isAuth('web:villagerule:add')&&deptLevel==5">新增</el-button>
      ...
      <el-table-column label="操作" width="180">
        <template slot-scope="scope">
          <div>
            ...
            <el-button type="text" size="small" @click="handleEdit(scope.row)" style="color:#E6A23C" v-if="isAuth('web:villagerule:edit')&&deptLevel==5">编辑</el-button>
  ...
  methods: {
    ...
    handleDelete(item) {
      this.$confirm("删除后不可恢复，您确定要继续吗?", "删除提示", {
        ...
        .then(async (_) => {
          const res = await this.$http.villageRuleDelete({
            id: item.id,
          });
          if (res.code == 0) {
            this.$message({
    ...
    async getListData() {
      ...
      const res = await this.$http.villageRuleList({
