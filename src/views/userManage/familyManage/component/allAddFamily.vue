<template>
  <div class="fixedBg">
    <div class="po-absolute modelMess modelMess1">
      ...
      <!--当前手机号-->
      <el-form ref="editChangeMobileForms" :model="addFamilyForm" :rules="familyRules" label-width="100px">
        ...
        <el-form-item label="添加数量：" prop="familycount">
          <el-input v-model="addFamilyForm.familycount" placeholder="请输入添加数量" oninput="value=value.replace(/[^\d]/g,'')">
            <template slot="append">户</template>
          </el-input>
        </el-form-item>

        <el-form-item style="text-align: center;position:relative;left: -45px">
          <el-button size="medium" @click="closeFamily('editChangeMobileForms')" style="margin-right: 30px">取消</el-button>
          <el-button type="primary" size="medium" @click="submitFamily('editChangeMobileForms')">批量生成</el-button>
        </el-form-item>
      </el-form>
    </div>
  </div>
  ...
  data() {
    ...
    return {
      ...
      familyRules: {
        ...
        familycount: [{ trigger: "blur", validator: isMax,required: true, }],
      },
      villageCode: "",
      adminUserInfo: "",
      groupList: [],
