...
<script>
    ...
    export default {
        ...
        methods:{
            //详情
            async getInfo(){
                let res = await this.$http.getCardById({id:this.$route.params.id});
                if(res.status==='success'){
                    this.depositForm.orderNum = res.data.orderNum
                    this.depositForm.cardName = res.data.cardName
                    this.depositForm.expireTime = res.data.expireTime
                    this.depositForm.score = res.data.score
                    this.depositForm.isBase = res.data.isBase
                    this.depositForm.isDonate = res.data.isDonate
                    if (res.data.icon) {
                        this.depositForm.picListUrls = res.data.icon
                    ...
                    if (res.data.background) {
                        this.depositForm.picBgUrls = res.data.background
            ...
            submitForm(){
                this.$refs['depositForm'].validate(async (valid) => {
                    ...
                    if (valid) {
                        this.isdisable = true
                        await this.$http.cardUpdate(data).then(res => {
                            if (res.code === '0') {
                                this.$message({
                                    type: 'success',
                                    message: res.msg
